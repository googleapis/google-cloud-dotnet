...
namespace Google.Cloud.Tools.ReleaseManager
{
    public sealed class PushCommand : CommandBase
    {
        ...
        private string PushBranch(Repository repo, Remote origin, string gitHubToken)
        {
            ...
            var remoteBranchName = $"release-pr-{DateTime.UtcNow:yyyy-MM-ddTHHmm'Z'}";
            ...
            repo.Network.Push(origin, $"{currentBranch.FriendlyName}:refs/heads/{remoteBranchName}", new PushOptions { CredentialsProvider = credentials });
        ...
        private void CreatePullRequest(Repository repo, GitHubClient gitHubClient, Remote origin, string branch)
        {
            ...
            async Task CreatePullRequestAsync()
            {
                ...
                Console.WriteLine($"Done! When the following pull request is is reviewed and merged, it will be automatically released:");
                // We put the pull request URL on a line on its own, 
