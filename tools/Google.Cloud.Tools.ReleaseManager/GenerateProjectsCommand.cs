...
namespace Google.Cloud.Tools.ReleaseManager
{
    public class GenerateProjectsCommand : CommandBase
    {
        ...
        protected override void ExecuteImpl(string[] args)
        {
            ...
            var owlBotCapableApis = RewriteOwlBotYaml(catalog);
            ...
            foreach (var api in catalog.Apis)
            {
                ...
                if (!owlBotCapableApis.Contains(api.Id))
                {
        ...
        /// <summary>
        /// Rewrites .github/.OwlBot.yaml to map all the OwlBot-capable APIs.
        /// This also deletes any synth.py/synth.metadata files.
        /// </summary>
        /// <returns>A set of all OwlBot-capable API IDs (to avoid revisiting when considering whether to generate synth configuration).</returns>
        private static HashSet<string> RewriteOwlBotYaml(ApiCatalog catalog)
        {
            // Parsing the YAML, manipulating it and then reserializing it (without losing formatting etc) is
            // likely to be fiddly at best. Instead, just split the file into "stuff that can be changed manually" and
            // "stuff below this line will be regenerated".
            const string warningLine = "# WARNING: Content below this line is regenerated by the project generator";
            string owlBotYamlFile = Path.Combine(DirectoryLayout.DetermineRootDirectory(), ".github", ".OwlBot.yaml");
            var oldOwlBotYamlLines = File.ReadAllLines(owlBotYamlFile).ToList();
            List<string> newOwlBotYamlLines = oldOwlBotYamlLines.TakeWhile(line => line != warningLine).ToList();
            newOwlBotYamlLines.Add(warningLine);
            newOwlBotYamlLines.Add("deep-copy-regex:");

            List <ApiMetadata> owlBotApis = new List<ApiMetadata>();           

            foreach (var api in catalog.Apis)
            {
                var apiRoot = DirectoryLayout.ForApi(api.Id).SourceDirectory;

                // Only GAPIC APIs that don't need any extra generator configuration
                // or proto patching can be generated by OwlBot.
                if (api.Generator != GeneratorType.Micro ||
                    api.CommonResourcesConfig is object ||
                    File.Exists(Path.Combine(apiRoot, "pregeneration.sh")) ||
                    File.Exists(Path.Combine(apiRoot, "midmicrogeneration.sh")))
                {
                    continue;
                }

                // Temporary measure: don't use OwlBot for APIs with postgeneration scripts
                // either. Given that it's postgeneration, we really should be able to do this
                // - but most postgeneration scripts expect googleapis to be present,
                // which it won't be for OwlBot. Skip for now, revisit later.
                if (File.Exists(Path.Combine(apiRoot, "postgeneration.sh")))
                {
                    continue;
                }

                // We don't use synthtool for OwlBot APIs
                File.Delete(Path.Combine(apiRoot, "synth.py"));
                File.Delete(Path.Combine(apiRoot, "synth.metadata"));

                // It's hard to predict exactly what the path will be, but we wouldn't
                // expect anything else to end in -csharp. Some APIs (e.g. KMS) have additional
                // redundant directories due to mixins; make sure we only copy the ones
                // we actually want.
                newOwlBotYamlLines.Add($" - source: /{api.ProtoPath}/.*-csharp/({api.Id}.*)");
                newOwlBotYamlLines.Add($"   dest: /owlbot-staging/{api.Id}/$1");
            }

            if (!oldOwlBotYamlLines.SequenceEqual(newOwlBotYamlLines))
            {
                Console.WriteLine("Rewriting OwlBot configuration");
                File.WriteAllLines(owlBotYamlFile, newOwlBotYamlLines);
            }

            return new HashSet<string>(owlBotApis.Select(api => api.Id));
        }
