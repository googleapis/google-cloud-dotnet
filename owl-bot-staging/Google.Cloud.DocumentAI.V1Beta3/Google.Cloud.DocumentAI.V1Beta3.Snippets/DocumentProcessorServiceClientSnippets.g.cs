    ...
    public sealed class AllGeneratedDocumentProcessorServiceClientSnippets
    {
        ...
        public void ProcessDocumentRequestObject()
        {
            ...
            gcdv::ProcessRequest request = new gcdv::ProcessRequest
            {
                ...
                ProcessOptions = new gcdv::ProcessOptions(),
            };
            // Make the request
        ...
        public void ImportProcessorVersionRequestObject()
        {
            ...
            gcdv::DocumentProcessorServiceClient documentProcessorServiceClient = gcdv::DocumentProcessorServiceClient.Create();
            // Initialize request argument(s)
            gcdv::ImportProcessorVersionRequest request = new gcdv::ImportProcessorVersionRequest
            {
                ParentAsProcessorName = gcdv::ProcessorName.FromProjectLocationProcessor("[PROJECT]", "[LOCATION]", "[PROCESSOR]"),
                ProcessorVersionSourceAsProcessorVersionName = gcdv::ProcessorVersionName.FromProjectLocationProcessorProcessorVersion("[PROJECT]", "[LOCATION]", "[PROCESSOR]", "[PROCESSOR_VERSION]"),
            };
            // Make the request
            Operation<gcdv::ImportProcessorVersionResponse, gcdv::ImportProcessorVersionMetadata> response = documentProcessorServiceClient.ImportProcessorVersion(request);

            // Poll until the returned long-running operation is complete
            Operation<gcdv::ImportProcessorVersionResponse, gcdv::ImportProcessorVersionMetadata> completedResponse = response.PollUntilCompleted();
            // Retrieve the operation result
            gcdv::ImportProcessorVersionResponse result = completedResponse.Result;

            // Or get the name of the operation
            string operationName = response.Name;
            // This name can be stored, then the long-running operation retrieved later by name
            Operation<gcdv::ImportProcessorVersionResponse, gcdv::ImportProcessorVersionMetadata> retrievedResponse = documentProcessorServiceClient.PollOnceImportProcessorVersion(operationName);
            // Check if the retrieved long-running operation has completed
            if (retrievedResponse.IsCompleted)
            {
                // If it has completed, then access the result
                gcdv::ImportProcessorVersionResponse retrievedResult = retrievedResponse.Result;
            }
            // End snippet
        }
        ...
        public async Task ImportProcessorVersionRequestObjectAsync()
        {
            ...
            // Make the request
            Operation<gcdv::ImportProcessorVersionResponse, gcdv::ImportProcessorVersionMetadata> response = await documentProcessorServiceClient.ImportProcessorVersionAsync(request);

            // Poll until the returned long-running operation is complete
            Operation<gcdv::ImportProcessorVersionResponse, gcdv::ImportProcessorVersionMetadata> completedResponse = await response.PollUntilCompletedAsync();
            // Retrieve the operation result
            gcdv::ImportProcessorVersionResponse result = completedResponse.Result;
            ...
            string operationName = response.Name;
            // This name can be stored, then the long-running operation retrieved later by name
            Operation<gcdv::ImportProcessorVersionResponse, gcdv::ImportProcessorVersionMetadata> retrievedResponse = await documentProcessorServiceClient.PollOnceImportProcessorVersionAsync(operationName);
            // Check if the retrieved long-running operation has completed
            if (retrievedResponse.IsCompleted)
            {
                // If it has completed, then access the result
        ...
        public void ImportProcessorVersion()
        {
            ...
            string parent = "projects/[PROJECT]/locations/[LOCATION]/processors/[PROCESSOR]";
            // Make the request
            Operation<gcdv::ImportProcessorVersionResponse, gcdv::ImportProcessorVersionMetadata> response = documentProcessorServiceClient.ImportProcessorVersion(parent);
        ...
        public async Task ImportProcessorVersionAsync()
        {
            ...
            string parent = "projects/[PROJECT]/locations/[LOCATION]/processors/[PROCESSOR]";
            // Make the request
            Operation<gcdv::ImportProcessorVersionResponse, gcdv::ImportProcessorVersionMetadata> response = await documentProcessorServiceClient.ImportProcessorVersionAsync(parent);
