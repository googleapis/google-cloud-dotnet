...
namespace GoogleCSharpSnippets
{
    ...
    public sealed partial class GeneratedApiHubClientSnippets
    {
        ...
        public void ListExternalApisRequestObject()
        {
            ...
            // Make the request
            PagedEnumerable<ListExternalApisResponse, ExternalApi> response = apiHubClient.ListExternalApis(request);

            // Iterate over all response items, lazily performing RPCs as required
            foreach (ExternalApi item in response)
            {
            ...
            foreach (ListExternalApisResponse page in response.AsRawResponses())
            {
                // Do something with each page of items
                Console.WriteLine("A page of results:");
                foreach (ExternalApi item in page)
                {
                    // Do something with each item
                    Console.WriteLine(item);
                }
            ...
            int pageSize = 10;
            Page<ExternalApi> singlePage = response.ReadPage(pageSize);
            // Do something with the page of items
            Console.WriteLine($"A page of {pageSize} results (unless it's the final page):");
            foreach (ExternalApi item in singlePage)
            {
                // Do something with each item
                Console.WriteLine(item);
            }
