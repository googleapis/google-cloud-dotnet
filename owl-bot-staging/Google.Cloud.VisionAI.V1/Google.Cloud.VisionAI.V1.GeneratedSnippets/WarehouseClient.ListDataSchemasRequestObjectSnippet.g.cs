...
namespace GoogleCSharpSnippets
{
    ...
    public sealed partial class GeneratedWarehouseClientSnippets
    {
        ...
        public void ListDataSchemasRequestObject()
        {
            ...
            // Make the request
            PagedEnumerable<ListDataSchemasResponse, DataSchema> response = warehouseClient.ListDataSchemas(request);

            // Iterate over all response items, lazily performing RPCs as required
            foreach (DataSchema item in response)
            {
            ...
            foreach (ListDataSchemasResponse page in response.AsRawResponses())
            {
            ...
            int pageSize = 10;
            Page<DataSchema> singlePage = response.ReadPage(pageSize);
