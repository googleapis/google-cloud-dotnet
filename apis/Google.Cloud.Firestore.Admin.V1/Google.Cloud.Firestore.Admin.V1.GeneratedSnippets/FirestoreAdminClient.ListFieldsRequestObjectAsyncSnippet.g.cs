...
namespace GoogleCSharpSnippets
{
    ...
    public sealed partial class GeneratedFirestoreAdminClientSnippets
    {
        ...
        public async Task ListFieldsRequestObjectAsync()
        {
            ...
            // Make the request
            PagedAsyncEnumerable<ListFieldsResponse, Field> response = firestoreAdminClient.ListFieldsAsync(request);

            // Iterate over all response items, lazily performing RPCs as required
            await foreach (Field item in response)
            {
            ...
            await foreach (ListFieldsResponse page in response.AsRawResponses())
            {
