...
namespace Google.Cloud.Run.V2
{
    ...
    {
        /// <summary><see cref="SecretName"/>-typed view over the <see cref="Secret"/> resource name property.</summary>
        public SecretName SecretAsSecretName
        {
            get => string.IsNullOrEmpty(Secret) ? null : SecretName.Parse(Secret, allowUnparsed: true);
            set => Secret = value?.ToString() ?? "";
        }

        /// <summary>
        /// <see cref="SecretVersionName"/>-typed view over the <see cref="Version"/> resource name property.
        /// </summary>
        public SecretVersionName VersionAsSecretVersionName
        {
            get => string.IsNullOrEmpty(Version) ? null : SecretVersionName.Parse(Version, allowUnparsed: true);
