...
public class SpannerBuiltInMetricsWrapperTests
{
    ...
    public void Properties_DelegateToInner()
    {
        ...
        Assert.Equal(mockClient.Settings, wrapper.Settings);
        Assert.Equal(mockClient.GrpcClient, wrapper.GrpcClient);
        Assert.Equal(mockClient.ClientIdentity.Id, wrapper.ClientIdentity.Id);
        Assert.Equal(mockClient.ClientIdentity.Hash, wrapper.ClientIdentity.Hash);
    }

    [Fact]
    public void MaybeWrap_Idempotent()
    {
        Assert.Null(SpannerBuiltInMetrics.BuiltInMetricsWrapper.MaybeWrap(null));

        var mockClient = Substitute.For<SpannerClient>();
        var wrapped = SpannerBuiltInMetrics.BuiltInMetricsWrapper.MaybeWrap(mockClient);
        Assert.IsType<SpannerBuiltInMetrics.BuiltInMetricsWrapper>(wrapped);

        var wrappedAgain = SpannerBuiltInMetrics.BuiltInMetricsWrapper.MaybeWrap(wrapped);
    ...
    public async Task ConsecutiveCalls_RecordsMultipleMetrics()
    {
        ...
        var scoped = FilterMeasurements(measurements, mockClient.ClientIdentity.Id);
        ...
        string expectedStatus)
    {
        ...
        var measurements = await RunWithMeterListenerAsync(() => invoke(wrapper));

        AssertRecordedData(measurements, mockClient.ClientIdentity.Id, expectedMethod, expectedStatus);
    }
