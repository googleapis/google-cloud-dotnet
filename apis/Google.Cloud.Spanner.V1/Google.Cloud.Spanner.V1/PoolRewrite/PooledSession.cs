...
namespace Google.Cloud.Spanner.V1.PoolRewrite
{
    ...
    public sealed class PooledSession
    {
        ...
        /// </summary>
        /// <remarks>
        /// This method does not affect <see cref="TransactionId"/> of this object.
        /// </remarks>
        ...
        /// <param name="cancellationToken">An optional token for canceling the call.</param>
        /// <returns>A task representing the asynchronous operation. When the task completes, the result is the response from the RPC.</returns>
        public Task<Transaction> BeginTransactionAsync(BeginTransactionRequest request, int timeoutSeconds, CancellationToken cancellationToken)
        {
            CheckNotDisposed();
            GaxPreconditions.CheckNotNull(request, nameof(request));
            request.SessionAsSessionName = SessionName;
        ...
        private async Task<T> RecordSuccessAndExpiredSessions<T>(Task<T> task, [CallerMemberName] string caller = null)
        {
            ...
            log.AppendLine($"Start {caller} at {DateTime.UtcNow:HH:mm:ss.fff}");
            log.AppendLine($"  Session={SessionName.SessionId}. TransactionId={TransactionId?.ToBase64()}");
            try
            {
                var result = await task.WithSessionChecking(_session).ConfigureAwait(false);
                log.AppendLine($"  Success at {DateTime.UtcNow:HH:mm:ss.fff}");
                UpdateRefreshTime();
                return result;
            }
            catch (RpcException e)
            {
                log.AppendLine($"  Failure at {DateTime.UtcNow:HH:mm:ss.fff}: {e.StatusCode} / {e.Message}");
                throw;
            }
            finally
            {
                //_pool.Logger.Debug(log.ToString().TrimEnd());
            }
        }
