...
namespace Google.Cloud.Spanner.V1
{
    ...
    public sealed class PooledSession : IDisposable
    {
        ...
        private readonly Func<Task<ManagedTransaction>> _managedTransactionFactory;
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        internal PooledSession(ManagedTransaction managedTransaction, Func<Task<ManagedTransaction>> managedTransactionFactory)
        {
            _managedTransaction = GaxPreconditions.CheckNotNull(managedTransaction, nameof(managedTransaction));
#pragma warning disable CS0618 // Type or member is obsolete
            GaxPreconditions.CheckArgument(
                _managedTransaction.Shared == _managedTransactionFactory is null,
                nameof(managedTransactionFactory),
                "A shared transaction cannot be refreshed." +
                "A transationc that's not shared may be refreshed." +
                "This is a library code bug.");
#pragma warning restore CS0618 // Type or member is obsolete
            _managedTransactionFactory = managedTransactionFactory;
        }
        ...
        /// </remarks>
        public async Task<PooledSession> RefreshedOrNewAsync(CancellationToken cancellationToken)
        {
#pragma warning disable CS0618 // Type or member is obsolete
            if (_managedTransaction.Shared)
            {
                throw new InvalidOperationException(
                        $"This session can't can't be refreshed and it can't be substituted by a new session. This is a bug in library code.");
            }
#pragma warning restore CS0618 // Type or member is obsolete
            return new PooledSession(await _managedTransactionFactory().ConfigureAwait(false), _managedTransactionFactory);
        }
