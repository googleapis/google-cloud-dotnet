...
internal static partial class SpannerBuiltInMetrics
{
    ...
    internal class BuiltInMetricsWrapper : SpannerClient
    {
        ...
        ///<inheritdoc/>
        public override SpannerSettings Settings => _inner.Settings;

        ///<inheritdoc/>
        internal override ClientIdentity ClientIdentity => _inner.ClientIdentity;
        ...
        /// <summary>
        /// Wraps the client in a <see cref="BuiltInMetricsWrapper"/> if it is not already wrapped.
        /// </summary>
        /// <param name="client">The client to wrap.</param>
        /// <returns>The wrapped client, or the original client if already wrapped or null.</returns>
        internal static SpannerClient MaybeWrap(SpannerClient client) =>
            client is null || client is BuiltInMetricsWrapper
                ? client
                : new BuiltInMetricsWrapper(client);
        ...
        // Streaming calls manage their own instrumentation and metric recording.
        ...
        private static StatusCode GetStatus(Exception ex) => ex switch
        {
        ...
        /// </summary>
        private void RecordMetrics(string method, object request, StatusCode status, double latencyMs)
        {
            var provider = request as IDatabaseNameProvider;
            Labeler.RecordOperationMetrics(latencyMs, method, provider, status, ClientIdentity);
        }
    }
}
