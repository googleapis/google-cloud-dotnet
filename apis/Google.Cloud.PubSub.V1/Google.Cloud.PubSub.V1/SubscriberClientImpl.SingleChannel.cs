...
public sealed partial class SubscriberClientImpl
{
    ...
    private class SingleChannel
    {
        ...
        private readonly object _lock = new object(); // For: _ackQueue, _nackQueue, _userHandlerInFlight, _shutdownNackedIds
        ...
        private readonly CancellationTokenSource _pushCts; // Cancels waiting for the next ACK/NACK/ModACK to be pushed to the server.
        private readonly CancellationTokenSource _pullCts; // Cancels the streaming pull, prevent new messages from being consumed.
        private readonly CancellationTokenSource _streamingPingCts; // Cancels the periodic streaming ping that is used to keeps the stream alive.
        private readonly CancellationTokenSource _leaseExtensionCts; // Cancels message lease extension operations.
        private readonly CancellationTokenSource _messageHandlerCts; // Cancels message handler operations.
        private readonly CancellationTokenSource _shutdownStartedCts; // Cancels operations that should stop immediately shutdown starts.
            ...
            IClock clock)
        {
            ...
            _pushCts                = CancellationTokenSource.CreateLinkedTokenSource(_hardStopCts.Token);
            _pullCts                = CancellationTokenSource.CreateLinkedTokenSource(subscriber._globalWaitForProcessingCts.Token, _softStopCts.Token, _nackImmediatelyCts.Token);
            _streamingPingCts       = CancellationTokenSource.CreateLinkedTokenSource(subscriber._globalWaitForProcessingCts.Token, _softStopCts.Token, _nackImmediatelyCts.Token);
            _leaseExtensionCts      = CancellationTokenSource.CreateLinkedTokenSource(_softStopCts.Token, _nackImmediatelyCts.Token);
            _messageHandlerCts      = CancellationTokenSource.CreateLinkedTokenSource(_nackImmediatelyCts.Token, _hardStopCts.Token);
            _shutdownStartedCts     = CancellationTokenSource.CreateLinkedTokenSource(_softStopCts.Token);
        ...
        internal async Task StartAsync()
        {
            ...
            while (!(_pullComplete && IsPushComplete()))
            {
                ...
                if (next.IsPull && (task.IsCanceled || (task.IsFaulted && (task.Exception.IsCancellation() || task.Exception.IsRpcCancellation())) || _pullCts.IsCancellationRequested))
                {
        ...
        private void HandlePullMessageData(Task<bool> moveNextTask)
        {
            ...
            if (moveNextTask.Result)
            {
                ...
                if (_exactlyOnceDeliveryEnabled)
                {
                    ...
                    Add(_eventReceiptModAckForExactlyOnceDelivery.WaitAsync(_shutdownStartedCts.Token)
        ...
        private async Task ProcessPullMessagesAsync(List<ReceivedMessage> msgs, HashSet<string> leaseTracking)
        {
            ...
            for (int msgIndex = 0; msgIndex < msgs.Count; msgIndex++)
            {
                ...
                {
                    // If NackImmediately has been triggered we stop processing new messages.
                    break;
                }
                ...
                await _taskHelper.ConfigureAwait(_flow.Process(msg.CalculateSize(), _messageOrderingEnabled ? msg.Message.OrderingKey ?? "" : "", async () =>
                {
                    ...
                    var reply = await _taskHelper.ConfigureAwaitHideErrors(() =>
                    {
                        ...
                        return _handler.HandleMessage(msg.Message, _messageHandlerCts.Token);
                    }, Reply.Nack);
        ...
        private void HandleExtendLease(HashSet<string> msgIds, LeaseCancellation cancellation)
        {
            if (_nackImmediatelyCts.IsCancellationRequested)
            {
                // No further lease extensions once stop is requested, instead NackEverything.
            ...
            if (cancellation == null)
            {
                ...
                cancellation = new LeaseCancellation(_leaseExtensionCts);
            ...
            if (!cancellation.IsDisposed)
            {
                ...
                if (anyMsgIds)
                {
                    ...
                    Add(_scheduler.Delay(EffectiveLeaseTiming.AutoExtendDelay, _leaseExtensionCts.Token), Next(false, () => HandleExtendLease(msgIds, cancellation)));
                    // Increment _extendThrottles.
                    _extendThrottleHigh += 1;
                    Add(_scheduler.Delay(EffectiveLeaseTiming.ExtendQueueThrottleInterval, _leaseExtensionCts.Token), Next(false, () => _extendThrottleLow += 1));
                }
                else
                {
        ...
        private void HandleAckResponseForExactlyOnceDelivery(Task writeTask, List<string> ackIds, List<string> nackIds, List<TimedId> extendIds)
        {
                ...
                int timeoutInSeconds = 600)
            {
                ...
                foreach (var retryGroup in retryGroups)
                {
                    ...
                    Task delayTask = _scheduler.Delay(backoff, _shutdownStartedCts.Token);
        ...
        private void HandleAckResponse(Task writeTask, List<string> ackIds, List<string> nackIds, List<TimedId> extendIds)
        {
            ...
            IReadOnlyList<AckNackResponse> GetAckNackResponses(List<string> ids, Task writeTask)
            {
                ...
                // Updates response statuses to failure if the messages were Nacked because of NackImmediately shutdown.
                ...
                {
                    // Only process if a NackImmediately shutdown is in progress
                    if (_nackImmediatelyCts.IsCancellationRequested)
                    {
        ...
        private void HandleStreamPing()
        {
            ...
            if (_streamingPingCts.IsCancellationRequested)
            {
                // No more pings when subscriber stopping.
                return;
            }
            // Schedule next ping, this never stops whilst this subscriber as active
            Add(_scheduler.Delay(s_streamPingPeriod, _streamingPingCts.Token), Next(false, HandleStreamPing));
