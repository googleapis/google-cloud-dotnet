...
public sealed partial class SubscriberClientImpl
{
    ...
    private class SingleChannel
    {
        ...
        private readonly int _modifyDeadlineSeconds; // Seconds to add to deadline on lease extension.
        private readonly int _modifyDeadlineSecondsForExactlyOnceDelivery; // Seconds to add to deadline on lease extension for exactly-once delivery subscriptions.
            ...
            Action<Task> registerTaskFn)
        {
            ...
            _modifyDeadlineSecondsForExactlyOnceDelivery = subscriber._modifyDeadlineSecondsForExactlyOnceDelivery;
            ...
            _extendQueueThrottleInterval = TimeSpan.FromTicks((long)((TimeSpan.FromSeconds(_modifyDeadlineSeconds) - _autoExtendDelay).Ticks * 0.5));
            _extendQueueThrottleIntervalForExactlyOnceDelivery = TimeSpan.FromTicks((long) ((TimeSpan.FromSeconds(_modifyDeadlineSecondsForExactlyOnceDelivery) - _autoExtendDelayForExactlyOnceDelivery).Ticks * 0.5));
        ...
        private void HandleStartStreamingPullWithoutBackoff()
        {
            ...
            Task initTask = _pull.WriteAsync(new StreamingPullRequest
            {
                ...
                StreamAckDeadlineSeconds = _exactlyOnceDeliveryEnabled ? _modifyDeadlineSecondsForExactlyOnceDelivery : _modifyDeadlineSeconds,
        ...
        private void HandleExtendLease(HashSet<string> msgIds, LeaseCancellation cancellation)
        {
            ...
            if (!cancellation.IsDisposed)
            {
                ...
                if (anyMsgIds)
                {
                    ...
                    Add(_scheduler.Delay(_exactlyOnceDeliveryEnabled ? _extendQueueThrottleInterval : _extendQueueThrottleIntervalForExactlyOnceDelivery, _softStopCts.Token), Next(false, () => _extendThrottleLow += 1));
                }
                else
                {
        ...
        private void StartPush()
        {
            ...
            if (extends.Count > 0)
            {
                ...
                _concurrentPushCount += 1;
                Task extendTask = _client.ModifyAckDeadlineAsync(_subscriptionName, extends.Select(x => x.Id), _exactlyOnceDeliveryEnabled ? _modifyDeadlineSecondsForExactlyOnceDelivery : _modifyDeadlineSeconds, _hardStopCts.Token);
