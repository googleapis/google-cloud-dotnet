    ...
    public sealed class PublisherClientImpl : PublisherClient
    {
        ...
        private void TriggerSend()
        {
            ...
            async Task Send()
            {
                ...
                List<Batch> batchesToCancel = null;
                ...
                lock (_lock)
                {
                    ...
                    state.SetState(OrderingKeyState.Normal);
                    var batches = state.Batches;
                    if (state.HasOrderingKey && publishTask.Status == TaskStatus.Faulted)
                    {
                        ...
                        {
                            // Prepare to cancel all batches, clear all batches, and mark ordering-key as in error state.
                            batchesToCancel = batches.ToList();
                            batches.Clear();
                ...
                if (batchesToCancel != null)
                {
                    // Cancel batches outside of the lock. These may be executed synchronously.
                    foreach (var batchToCancel in batchesToCancel)
                    {
                        batchToCancel.BatchCompletion.SetCanceled();
                    }
                }
            }
        }
