...
namespace Google.Cloud.PubSub.V1.Tests
{
    public class SubscriberClientTest
    {
        ...
        [Theory, CombinatorialData]
        public void Shutdown_ServerFaultsUnrecoverable(ShutdownMode shutdownMode)
        {
        ...
        public void LeaseMaxExtension(bool isExactlyOnceDelivery, ShutdownMode shutdownMode)
        {
            ...
            using (var fake = Fake.Create(msgs, isExactlyOnceDelivery: isExactlyOnceDelivery, ackDeadline: TimeSpan.FromSeconds(30), ackExtendWindow: TimeSpan.FromSeconds(10)))
            {
                fake.Scheduler.Run(async () =>
                {
                    ...
                    await fake.TaskHelper.ConfigureAwait(fake.Subscriber.StopAsync(new ShutdownOptions { Mode = shutdownMode, Timeout = TimeSpan.FromHours(24) }));
                    await fake.TaskHelper.ConfigureAwait(doneTask);
                    Assert.Equal(1, fake.Subscribers.Count);
                ...
                fake.Scheduler.Run(async () =>
                {
                    ...
                    var startTask = fake.Subscriber.StartAsync(async (msg, ct) =>
                    {
                        ...
                        lock (recvedMsgs)
                        {
                            ...
                            {
                                var dummyTask = fake.Subscriber.StopAsync(new ShutdownOptions { Mode = ShutdownMode.WaitForProcessing, Timeout = TimeSpan.FromHours(24) });
                            }
                        }
                        return SubscriberClient.Reply.Ack;
                    });
        ...
        public void Shutdown_SoftStop_NacksMessages()
        {
            ...
            // flowMaxElements=2: msg0 and msg4 block flow control; msg1, msg2, msg3 are processed quickly.
            // msg5-6 wait for flow control and should be Nacked on shutdown.
            ...
            fake.Scheduler.Run(async () =>
            {
                ...
                // msg0-4 were handled/acked; msg5-6 were pulled but nacked on shutdown.
