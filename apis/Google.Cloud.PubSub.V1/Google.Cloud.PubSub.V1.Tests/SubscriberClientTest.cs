...
using Xunit;
using ShutdownOption = Google.Cloud.PubSub.V1.SubscriberClient.ShutdownOption;
...
namespace Google.Cloud.PubSub.V1.Tests
{
    public class SubscriberClientTest
    {
        ...
        {
            private readonly Func<PubsubMessage, CancellationToken, Task<SubscriberClient.Reply>> _handler;

            internal List<AckNackResponse> Responses { get; } = new List<AckNackResponse>();

            internal TestSubscriptionHandler(Func<PubsubMessage, CancellationToken, Task<SubscriberClient.Reply>> handler) => _handler = handler;

            internal TestSubscriptionHandler(bool ackOrNack) => _handler = (msg, ct) => Task.FromResult(ackOrNack ? SubscriberClient.Reply.Ack : SubscriberClient.Reply.Nack);
        ...
        public void ImmediateStop(
            [CombinatorialValues(false, true)] bool hardStop,
            ShutdownOption shutdownSetting)
        {
            using (var fake = Fake.Create(new[] { new[] { ServerAction.Inf() } }))
            {
                fake.Scheduler.Run(async () =>
                {
                    ...
                    await fake.TaskHelper.ConfigureAwait(fake.Scheduler.Delay(TimeSpan.FromSeconds(1), CancellationToken.None));
                    var isCancelled = await fake.TaskHelper.ConfigureAwaitHideCancellation(
                        () => fake.Subscriber.StopAsync(shutdownSetting, cancellationToken: new CancellationToken(hardStop)));
                    Assert.Equal(hardStop, isCancelled);
                    Assert.Equal(1, fake.Subscribers.Count);
                    Assert.Empty(fake.Subscribers[0].Acks);
                    Assert.Empty(fake.Subscribers[0].Nacks);
        ...
        [Theory, CombinatorialData]
        public void StopBeforeStart(ShutdownOption shutdownSetting)
        {
                ...
                fake.Scheduler.Run(async () =>
                {
                    ...
                    var exception = await Assert.ThrowsAsync<InvalidOperationException>(
                        async () => await fake.TaskHelper.ConfigureAwait(
                            fake.Subscriber.StopAsync(shutdownSetting, TimeSpan.FromHours(1))));
        ...
        [Theory, CombinatorialData]
        public void Dispose(ShutdownOption shutdownSetting)
        {
        ...
        [Theory, PairwiseData]
        public void RecvManyMsgsNoErrors_Obsolete(
            [CombinatorialValues(false, true)] bool hardStop,
        ...
        public void RecvManyMsgsNoErrors(
            ...
            [CombinatorialValues(1, 2, 3, 4, 8, 16, 33)] int clientCount)
        {
            int delayToSubtract = shutdownSetting switch
            {
                ShutdownOption.WaitForProcessing => hardStop ? handlerDelaySeconds : 0,
                ShutdownOption.NackImmediately => handlerDelaySeconds,
                ...
                fake.Scheduler.Run(async () =>
                {
                    ...
                    var startTask = fake.Subscriber.StartAsync(async (msg, ct) =>
                    {
                        ...
                        lock (recvedMsgs)
                        {
                            ...
                            {
                                Task unused = fake.Subscriber.StopAsync(ShutdownOption.WaitForProcessing);
                            }
                        }
                        return SubscriberClient.Reply.Ack;
                    });
        ...
        [Theory, PairwiseData]
        public void FlowControl_Obsolete(
            [CombinatorialValues(false, true)] bool hardStop,
            ...
            [CombinatorialValues(1, 3, 9, 19)] int threadCount)
        {
                ...
                flowMaxElements: flowMaxElements, flowMaxBytes: flowMaxBytes, clientCount: clientCount, threadCount: threadCount))
            {
                fake.Scheduler.Run(async () =>
                {
                    ...
                    await fake.TaskHelper.ConfigureAwait(fake.Scheduler.Delay(TimeSpan.FromSeconds(stopAfterSeconds) + TimeSpan.FromSeconds(0.5), CancellationToken.None));
                    await fake.TaskHelper.ConfigureAwaitHideCancellation(() => fake.Subscriber.StopAsync(ShutdownOption.NackImmediately, cancellationToken: new CancellationToken(hardStop)));
                    Assert.Equal(expectedMsgCount, handledMsgs.Count);
                });
            }
        }
        ...
        [Theory, CombinatorialData]
        public void UserHandlerFaults(ShutdownOption shutdownSetting)
        {
            ...
            using (var fake = Fake.Create(new[] { msgs }))
            {
                fake.Scheduler.Run(async () =>
                {
                    ...
                    await fake.TaskHelper.ConfigureAwait(fake.Subscriber.StopAsync(shutdownSetting));
            ...
            ShutdownOption shutdownSetting)
        {
            ...
            using (var fake = Fake.Create(msgs, clientCount: clientCount, threadCount: threadCount, flowMaxElements: 1))
            {
                fake.Scheduler.Run(async () =>
                {
                    ...
                    await fake.TaskHelper.ConfigureAwait(fake.Scheduler.Delay(TimeSpan.FromSeconds(10), CancellationToken.None));
                    Exception ex = await fake.TaskHelper.ConfigureAwaitHideErrors(() => fake.Subscriber.StopAsync(shutdownSetting));
        ...
        [Theory, CombinatorialData]
        public void Shutdown_ServerFaultsUnrecoverable(ShutdownOption shutdownSetting)
        {
            // Ensure StopAsync propagates unrecoverable server errors.
            var zero = TimeSpan.Zero;
            var unrecoverableEx = new RpcException(new Status(StatusCode.Unimplemented, ""), "");
            var failure = ServerAction.BadMoveNext(zero, unrecoverableEx);
            var msgs = new[] { new[] { ServerAction.Data(zero, new[] { "1", "2" }), failure } };

            using (var fake = Fake.Create(msgs, flowMaxElements: 1))
            {
                fake.Scheduler.Run(async () =>
                {
                    var handledMsgs = new List<string>();
                    ...
                    // Stop the subscriber after a server failure has occurred.
                    Exception ex = await fake.TaskHelper.ConfigureAwaitHideErrors(() => fake.Subscriber.StopAsync(shutdownSetting));

                    // The resulting task should fault with the original server exception.
                    Assert.Equal(unrecoverableEx, ex.AllExceptions().FirstOrDefault());
                    Assert.NotEmpty(handledMsgs);
                    Assert.Equal(1, fake.ClientShutdowns.Count);
                });
        ...
        [Theory, CombinatorialData]
        public void LeaseExtension(bool isExactlyOnceDelivery, ShutdownOption shutdownSetting)
        {
            var msgs = new[] { new[] {
        ...
        [Theory, CombinatorialData]
        public void LeaseMaxExtension(bool isExactlyOnceDelivery, ShutdownOption shutdownSetting)
        {
            var msgs = new[] { new[] {
                ServerAction.Data(TimeSpan.Zero, new[] { "1" }),
                ServerAction.Inf()
            ...
            using (var fake = Fake.Create(msgs, isExactlyOnceDelivery: isExactlyOnceDelivery, ackDeadline: TimeSpan.FromSeconds(30), ackExtendWindow: TimeSpan.FromSeconds(10)))
            {
                fake.Scheduler.Run(async () =>
                {
                    ...
                    await fake.TaskHelper.ConfigureAwait(fake.Subscriber.StopAsync(shutdownSetting, TimeSpan.FromHours(24)));
                    await fake.TaskHelper.ConfigureAwait(doneTask);
                    Assert.Equal(1, fake.Subscribers.Count);
        ...
        [Theory, CombinatorialData]
        public void SlowUplinkThrottlesPull(ShutdownOption shutdownSetting)
        {
        ...
        [Theory, CombinatorialData]
        public void StreamPings(ShutdownOption shutdownSetting)
        {
            ...
            using (var fake = Fake.Create(new[] { msgs }))
            {
                ...
                fake.Scheduler.Run(async () =>
                {
                    ...
                    // Stop subscriber.
                    await th.ConfigureAwait(fake.Subscriber.StopAsync(shutdownSetting));
                ...
                fake.Scheduler.Run(async () =>
                {
                    ...
                    var startTask = fake.Subscriber.StartAsync(async (msg, ct) =>
                    {
                        ...
                        lock (recvedMsgs)
                        {
                            ...
                            {
                                var dummyTask = fake.Subscriber.StopAsync(ShutdownOption.WaitForProcessing, TimeSpan.FromHours(24));
                            }
                        }
                        return SubscriberClient.Reply.Ack;
                    });
        ...
        [Theory, CombinatorialData]
        public void DeliveryAttempt(ShutdownOption shutdownSetting)
        {
            var msgs = new[] {
        ...
        [Theory, CombinatorialData]
        public void AckModifyAckDeadlineFault_NotThrown([CombinatorialValues(true, false, null)] bool? ackOrModifyAck, ShutdownOption shutdownSetting)
        {
            var msgs = new[]
            {
                ServerAction.Data(TimeSpan.Zero, new[] { "1" }),
        ...
        [Theory, CombinatorialData]
        public void AckModifyAckDeadlineFault_Thrown([CombinatorialValues(true, false)] bool ackOrModifyAck, ShutdownOption shutdownSetting)
        {
            var msgs = new[]
            {
                ServerAction.Data(TimeSpan.Zero, new[] { "1" }),
        ...
        [Theory, CombinatorialData]
        public void AckModifyAckDeadlineFault_SubscriptionHandler(bool ackOrModifyAck, ShutdownOption shutdownSetting)
        {
            var msgs = new[]
            {
                ServerAction.Data(TimeSpan.Zero, new[] { "1" }),
        ...
        [Theory, CombinatorialData]
        public void ExactlyOnceDelivery_TemporaryFault([CombinatorialValues(true, false, null)] bool? ackNackOrExtends, ShutdownOption shutdownSetting)
        {
            var msgs = new[]
            {
                ServerAction.Data(TimeSpan.Zero, new[] { "1" }),
        ...
        [Theory, CombinatorialData]
        public void ExactlyOnceDelivery_AckNack_PermanentFault([CombinatorialValues(true, false)] bool ackOrNack, ShutdownOption shutdownSetting)
        {
            var msgs = new[]
            {
                ServerAction.Data(TimeSpan.Zero, new[] { "1" }),
        ...
        [Theory, CombinatorialData]
        public void ExactlyOnceDelivery_Extends_PermanentFault(ShutdownOption shutdownSetting)
        {
            var msgs = new[]
            {
                ServerAction.Data(TimeSpan.Zero, new[] { "1" }),
        ...
        [Theory, CombinatorialData]
        public void ExactlyOnceDelivery_AckNack_MixedFault([CombinatorialValues(true, false)] bool ackOrNack, ShutdownOption shutdownSetting)
        {
            var msgs = new[]
            {
                ServerAction.Data(TimeSpan.Zero, new[] { "1" }),
        ...
        [Theory, CombinatorialData]
        public void ExactlyOnceDelivery_Extends_MixedFault(ShutdownOption shutdownSetting)
        {
            var msgs = new[]
            {
                ServerAction.Data(TimeSpan.Zero, new[] { "1" }),
        ...
        [Theory, CombinatorialData]
        public void ExactlyOnceDelivery_ReceiptModAck(ShutdownOption shutdownSetting)
        {
            var msgs = new[]
            {
                ServerAction.Data(TimeSpan.Zero, new[] { "1" }),
        ...
        [Theory, CombinatorialData]
        public void ExactlyOnceDelivery_ReceiptModAck_MixedFault([CombinatorialValues(true, false)] bool succeedOnRetry, ShutdownOption shutdownSetting)
        {
            var msgs = new[]
            {
                ServerAction.Data(TimeSpan.Zero, new[] { "1" }),
        ...
        [Theory, CombinatorialData]
        public void ExactlyOnceDelivery_ReceiptModAck_PermanentFaults(ShutdownOption shutdownSetting)
        {
            var msgs = new[]
            {
                ServerAction.Data(TimeSpan.Zero, new[] { "1" }),
        ...
        [Theory, CombinatorialData]
        public void ExactlyOnceDelivery_ReceiptModAck_TemporaryFaults([CombinatorialValues(true, false)] bool succeedOnRetry, ShutdownOption shutdownSetting)
        {
            var msgs = new[]
            {
                ServerAction.Data(TimeSpan.Zero, new[] { "1" }),
        ...
        [Theory, CombinatorialData]
        public void StreamingPullRetry_InternalErrorContinuesRetrying(ShutdownOption shutdownSetting)
        {
        ...
        /// </summary>
        [Theory, CombinatorialData]
        public void StreamingPullRetry_UnavailableAfterShortDelayTriggersRetryWithBackoff(ShutdownOption shutdownSetting)
        {
        ...
        /// </summary>
        [Theory, CombinatorialData]
        public void StreamingPullRetry_UnavailableAfterLongDelayTriggersRetryWithoutBackoff(ShutdownOption shutdownSetting)
        {
            var exception = new RpcException(new Status(StatusCode.Unavailable, "Stream terminated"));
        ...
        [Fact]
        public void Shutdown_SoftStop_NacksMessages()
        {
            var msgs = new[] {
                ServerAction.Data(TimeSpan.Zero, ["msg0"]),
                ServerAction.Data(TimeSpan.Zero, ["msg1", "msg2", "msg3"]),
                ServerAction.Data(TimeSpan.Zero, ["msg4", "msg5", "msg6"]),
                ServerAction.Data(TimeSpan.Zero, ["msg7"]),
                ServerAction.Inf()
            };
            // flowMaxElements=2: msg0 and msg2 block flow control; msg1 is processed quickly.
            // msg3-5 wait for flow control and should be Nacked on shutdown.
            ...
            fake.Scheduler.Run(async () =>
            {
                ...
                var startTask = fake.Subscriber.StartAsync(async (msg, ct) =>
                {
                    ...
                    if (data == "msg0" || data == "msg4")
                    {
                        await fake.TaskHelper.ConfigureAwait(fake.Scheduler.Delay(TimeSpan.FromMinutes(1), ct));
                    }
                    return SubscriberClient.Reply.Ack;
                });
                ...
                Assert.Equivalent(new[] { "msg5", "msg6" }, fake.Subscribers[0].Nacks.Select(x => x.Id), strict: true);
            });
        }
        ...
        public void Shutdown_NackImmediately_Success()
        {
            ...
            fake.Scheduler.Run(async () =>
            {
                ...
                await fake.TaskHelper.ConfigureAwait(fake.Subscriber.StopAsync(ShutdownOption.NackImmediately, TimeSpan.FromSeconds(60)));
        ...
        [Theory, CombinatorialData]
        public void Shutdown_NackImmediately_AckResponseStatus(bool isExactlyOnceDelivery)
        {
            var msgs = new[] {
                ServerAction.Data(TimeSpan.Zero, ["msg0"]),
                ServerAction.Data(TimeSpan.Zero, ["msg1"]),
                ServerAction.Inf()
            };
            // flowMaxElements=1: msg1 blocks flow control until msg0 is handled.
            using var fake = Fake.CreateClientForSingleResponseStream(msgs, flowMaxElements: 1, useMsgAsId: true, isExactlyOnceDelivery: isExactlyOnceDelivery);

            var testHandler = new TestSubscriptionHandler(async (msg, ct) =>
            {
                var data = msg.Data.ToStringUtf8();
                if (data == "msg1")
                {
                    // Delay msg1 to ensure it's in-flight when shutdown happens.
                    await fake.TaskHelper.ConfigureAwait(fake.Scheduler.Delay(TimeSpan.FromSeconds(30), ct));
                }
                return SubscriberClient.Reply.Ack;
            });
            ...
            fake.Scheduler.Run(async () =>
            {
                var startTask = fake.Subscriber.StartAsync(testHandler);

                // Wait for msg0 to be handled and msg1 to start being handled.
                await fake.TaskHelper.ConfigureAwait(fake.Scheduler.Delay(TimeSpan.FromSeconds(1), CancellationToken.None));

                // Stop immediately. This should forcefully nack msg1.
                await fake.TaskHelper.ConfigureAwait(fake.Subscriber.StopAsync(ShutdownOption.NackImmediately, TimeSpan.FromSeconds(60)));

                // Check captured responses.
                // msg0 should be Success as it was handled before shutdown.
                Assert.Contains(testHandler.Responses, r => r.MessageId == "msg0" && r.Status == AcknowledgementStatus.Success);
                // msg1 should be Other as it was nacked due to shutdown, even though the handler returned Ack.
                Assert.Contains(testHandler.Responses, r => r.MessageId == "msg1" && r.Status == AcknowledgementStatus.Other);
            });
        }
        ...
        public void Shutdown_WaitForProcessing_CompletesBeforeNack()
        {
            ...
            fake.Scheduler.Run(async () =>
            {
                ...
                await fake.TaskHelper.ConfigureAwait(fake.Subscriber.StopAsync(ShutdownOption.WaitForProcessing, TimeSpan.FromSeconds(60)));
        ...
        public void Shutdown_WaitForProcessing_NacksOnTimeout()
        {
            ...
            var msgs = new[] {
                ...
                ServerAction.Data(TimeSpan.Zero, ["msg5", "msg6"]),
                ServerAction.Inf()
            };
            using var fake = Fake.CreateClientForSingleResponseStream(msgs, flowMaxElements: 2, useMsgAsId: true);
            fake.Scheduler.Run(async () =>
            {
                ...
                // Timeout=45s: NackDelay=15s. At T=5+15=20s, NackImmediately is triggered.
                // msg1, msg2 finish at T=20s. msg3, msg4 start at T=20s but are Nacked as shutdown is in progress.
                await fake.TaskHelper.ConfigureAwait(fake.Subscriber.StopAsync(ShutdownOption.WaitForProcessing, TimeSpan.FromSeconds(50)));

                // Verify the switch to Nacking for the remaining messages.
        ...
        public void Shutdown_CancellationToken_AbortsWaitForProcessing()
        {
            ...
            fake.Scheduler.Run(async () =>
            {
                ...
                var stopTask = fake.Subscriber.StopAsync(ShutdownOption.WaitForProcessing, TimeSpan.FromHours(1), cts.Token);
                Assert.Empty(fake.Subscribers[0].Acks);
                Assert.Empty(fake.Subscribers[0].Nacks);
        ...
        public void Shutdown_WaitForProcessing_NacksWhenTimeoutLessThanMinimum()
        {
            ...
            fake.Scheduler.Run(async () =>
            {
                ...
                await fake.TaskHelper.ConfigureAwait(fake.Subscriber.StopAsync(ShutdownOption.WaitForProcessing, TimeSpan.FromSeconds(15)));
        ...
        public void Shutdown_NackImmediately_LeaseExtensionStops()
        {
            ...
            using (var fake = Fake.Create(msgs, ackDeadline: TimeSpan.FromSeconds(30), ackExtendWindow: TimeSpan.FromSeconds(10)))
            {
                fake.Scheduler.Run(async () =>
                {
                    ...
                    await fake.TaskHelper.ConfigureAwait(fake.Subscriber.StopAsync(ShutdownOption.NackImmediately));
