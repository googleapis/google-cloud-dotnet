...
namespace GoogleCSharpSnippets
{
    ...
    public sealed class AllGeneratedParticipantsClientSnippets
    {
        ...
        /// <summary>Snippet for BidiStreamingAnalyzeContent</summary>
        public async Task BidiStreamingAnalyzeContent()
        {
            // Snippet: BidiStreamingAnalyzeContent(CallSettings, BidirectionalStreamingSettings)
            // Create client
            ParticipantsClient participantsClient = ParticipantsClient.Create();
            // Initialize streaming call, retrieving the stream object
            using ParticipantsClient.BidiStreamingAnalyzeContentStream response = participantsClient.BidiStreamingAnalyzeContent();
            ...
            {
                // Note that C# 8 code can use await foreach
                AsyncResponseStream<BidiStreamingAnalyzeContentResponse> responseStream = response.GetResponseStream();
                while (await responseStream.MoveNextAsync())
                {
                    BidiStreamingAnalyzeContentResponse responseItem = responseStream.Current;
                    // Do something with streamed response
                }
                // The response stream has completed
            });
            ...
            {
                // Initialize a request
                BidiStreamingAnalyzeContentRequest request = new BidiStreamingAnalyzeContentRequest
                {
                    Config = new BidiStreamingAnalyzeContentRequest.Types.Config(),
                };
                // Stream a request to the server
                await response.WriteAsync(request);
                // Set "done" to true when sending requests is complete
        ...
        /// <summary>Snippet for StreamingReactiveCompanionSuggestions</summary>
        public async Task StreamingReactiveCompanionSuggestions()
        {
            // Snippet: StreamingReactiveCompanionSuggestions(CallSettings, BidirectionalStreamingSettings)
            // Create client
            ParticipantsClient participantsClient = ParticipantsClient.Create();
            // Initialize streaming call, retrieving the stream object
            using ParticipantsClient.StreamingReactiveCompanionSuggestionsStream response = participantsClient.StreamingReactiveCompanionSuggestions();
            ...
            {
                // Note that C# 8 code can use await foreach
                AsyncResponseStream<StreamingReactiveCompanionSuggestionsResponse> responseStream = response.GetResponseStream();
                while (await responseStream.MoveNextAsync())
                {
                    StreamingReactiveCompanionSuggestionsResponse responseItem = responseStream.Current;
                    // Do something with streamed response
                }
                // The response stream has completed
            });
            ...
            {
                // Initialize a request
                StreamingReactiveCompanionSuggestionsRequest request = new StreamingReactiveCompanionSuggestionsRequest
                {
                    ParticipantAsParticipantName = ParticipantName.FromProjectConversationParticipant("[PROJECT]", "[CONVERSATION]", "[PARTICIPANT]"),
                    TextInput = "",
                };
