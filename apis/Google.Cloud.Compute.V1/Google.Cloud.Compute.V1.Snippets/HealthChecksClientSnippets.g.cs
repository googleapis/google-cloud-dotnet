...
namespace GoogleCSharpSnippets
{
    ...
    public sealed class AllGeneratedHealthChecksClientSnippets
    {
        ...
        public async Task AggregatedListRequestObjectAsync()
        {
            ...
            // Iterate over all response items, lazily performing RPCs as required
            await foreach (KeyValuePair<string, HealthChecksScopedList> item in response)
            {
                // Do something with each item
                Console.WriteLine(item);
            }
            ...
            // Or iterate over pages (of server-defined size), performing one RPC per page
            await foreach (HealthChecksAggregatedList page in response.AsRawResponses())
            {
                // Do something with each page of items
                Console.WriteLine("A page of results:");
                foreach (KeyValuePair<string, HealthChecksScopedList> item in page)
        ...
        public async Task ListRequestObjectAsync()
        {
            ...
            // Iterate over all response items, lazily performing RPCs as required
            await foreach (HealthCheck item in response)
            {
                // Do something with each item
                Console.WriteLine(item);
            }
            ...
            // Or iterate over pages (of server-defined size), performing one RPC per page
            await foreach (HealthCheckList page in response.AsRawResponses())
            {
                // Do something with each page of items
                Console.WriteLine("A page of results:");
                foreach (HealthCheck item in page)
