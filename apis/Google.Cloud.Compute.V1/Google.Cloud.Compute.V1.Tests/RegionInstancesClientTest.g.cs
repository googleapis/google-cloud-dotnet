...
namespace Google.Cloud.Compute.V1.Tests
{
    ...
    public sealed class GeneratedRegionInstancesClientTest
    {
        [xunit::FactAttribute]
        ...
        public async stt::Task BulkInsertAsync()
        {
            ...
            mockGrpcClient.Setup(x => x.BulkInsertAsync(request, moq::It.IsAny<grpccore::CallOptions>())).Returns(new grpccore::AsyncUnaryCall<Operation>(stt::Task.FromResult(expectedResponse), null, null, null, null));
            RegionInstancesClient client = new RegionInstancesClientImpl(mockGrpcClient.Object, null);
            Operation responseCallSettings = await client.BulkInsertAsync(request.Project, request.Region, request.BulkInsertInstanceResourceResource, gaxgrpc::CallSettings.FromCancellationToken(st::CancellationToken.None));
            xunit::Assert.Same(expectedResponse, responseCallSettings);
            Operation responseCancellationToken = await client.BulkInsertAsync(request.Project, request.Region, request.BulkInsertInstanceResourceResource, st::CancellationToken.None);
            xunit::Assert.Same(expectedResponse, responseCancellationToken);
            mockGrpcClient.VerifyAll();
        }
    }
