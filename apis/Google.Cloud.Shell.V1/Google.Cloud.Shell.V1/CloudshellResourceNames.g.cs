...
namespace Google.Cloud.Shell.V1
{
    ...
    public sealed partial class EnvironmentName : gax::IResourceName, sys::IEquatable<EnvironmentName>
    {
        ...
        public enum ResourceNameType
        {
            ...
            /// <summary>A resource name with pattern <c>users/{user}/environments/{environment}</c>.</summary>
            UserEnvironment = 1,
        }

        private static gax::PathTemplate s_userEnvironment = new gax::PathTemplate("users/{user}/environments/{environment}");
        ...
        /// <summary>
        /// Creates a <see cref="EnvironmentName"/> with the pattern <c>users/{user}/environments/{environment}</c>.
        /// </summary>
        ...
        public static EnvironmentName FromUserEnvironment(string userId, string environmentId) =>
            new EnvironmentName(ResourceNameType.UserEnvironment, userId: gax::GaxPreconditions.CheckNotNullOrEmpty(userId, nameof(userId)), environmentId: gax::GaxPreconditions.CheckNotNullOrEmpty(environmentId, nameof(environmentId)));

        /// <summary>
        /// Formats the IDs into the string representation of this <see cref="EnvironmentName"/> with pattern
        /// <c>users/{user}/environments/{environment}</c>.
        /// </summary>
        /// <param name="userId">The <c>User</c> ID. Must not be <c>null</c> or empty.</param>
        /// <param name="environmentId">The <c>Environment</c> ID. Must not be <c>null</c> or empty.</param>
        /// <returns>
        ...
        /// <c>users/{user}/environments/{environment}</c>.
        /// </returns>
        public static string Format(string userId, string environmentId) => FormatUserEnvironment(userId, environmentId);
        ...
        public static string FormatUserEnvironment(string userId, string environmentId) =>
            s_userEnvironment.Expand(gax::GaxPreconditions.CheckNotNullOrEmpty(userId, nameof(userId)), gax::GaxPreconditions.CheckNotNullOrEmpty(environmentId, nameof(environmentId)));

        /// <summary>Parses the given resource name string into a new <see cref="EnvironmentName"/> instance.</summary>
        /// <remarks>
        /// To parse successfully, the resource name must be formatted as one of the following:
        /// <list type="bullet">
        /// <item><description><c>users/{user}/environments/{environment}</c></description></item>
        /// </list>
        /// </remarks>
        ...
        public static bool TryParse(string environmentName, bool allowUnparsed, out EnvironmentName result)
        {
            ...
            if (s_userEnvironment.TryParseName(environmentName, out resourceName))
            {
                result = FromUserEnvironment(resourceName[0], resourceName[1]);
                return true;
            }
            if (allowUnparsed)
            {
        ...
        private EnvironmentName(ResourceNameType type, gax::UnparsedResourceName unparsedResourceName = null, string environmentId = null, string userId = null)
        {
            Type = type;
        ...
        /// <param name="userId">The <c>User</c> ID. Must not be <c>null</c> or empty.</param>
        /// <param name="environmentId">The <c>Environment</c> ID. Must not be <c>null</c> or empty.</param>
        public EnvironmentName(string userId, string environmentId) : this(ResourceNameType.UserEnvironment, userId: gax::GaxPreconditions.CheckNotNullOrEmpty(userId, nameof(userId)), environmentId: gax::GaxPreconditions.CheckNotNullOrEmpty(environmentId, nameof(environmentId)))
        {
        }
        ...
        public override string ToString()
        {
            switch (Type)
            {
                ...
                case ResourceNameType.UserEnvironment: return s_userEnvironment.Expand(UserId, EnvironmentId);
    ...
    {
        /// <summary>
        /// <see cref="gcsv::EnvironmentName"/>-typed view over the <see cref="Name"/> resource name property.
        /// </summary>
        public gcsv::EnvironmentName EnvironmentName
        {
            get => string.IsNullOrEmpty(Name) ? null : gcsv::EnvironmentName.Parse(Name, allowUnparsed: true);
