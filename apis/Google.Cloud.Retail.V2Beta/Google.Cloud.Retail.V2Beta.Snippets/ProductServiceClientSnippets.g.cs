...
namespace Google.Cloud.Retail.V2Beta.Snippets
{
    ...
    public sealed class GeneratedProductServiceClientSnippets
    {
        ...
        public void PurgeProductsRequestObject()
        {
            ...
            // Make the request
            Operation<PurgeProductsResponse, PurgeMetadata> response = productServiceClient.PurgeProducts(request);

            // Poll until the returned long-running operation is complete
            Operation<PurgeProductsResponse, PurgeMetadata> completedResponse = response.PollUntilCompleted();
            ...
            Operation<PurgeProductsResponse, PurgeMetadata> retrievedResponse = productServiceClient.PollOncePurgeProducts(operationName);
        ...
        public async Task PurgeProductsRequestObjectAsync()
        {
            ...
            // Make the request
            Operation<PurgeProductsResponse, PurgeMetadata> response = await productServiceClient.PurgeProductsAsync(request);

            // Poll until the returned long-running operation is complete
            Operation<PurgeProductsResponse, PurgeMetadata> completedResponse = await response.PollUntilCompletedAsync();
            // Retrieve the operation result
            PurgeProductsResponse result = completedResponse.Result;
            ...
            string operationName = response.Name;
            // This name can be stored, then the long-running operation retrieved later by name
            Operation<PurgeProductsResponse, PurgeMetadata> retrievedResponse = await productServiceClient.PollOncePurgeProductsAsync(operationName);
            // Check if the retrieved long-running operation has completed
            if (retrievedResponse.IsCompleted)
            {
                // If it has completed, then access the result
        ...
        /// <summary>Snippet for ExportProducts</summary>
        public void ExportProductsRequestObject()
        {
            // Snippet: ExportProducts(ExportProductsRequest, CallSettings)
            // Create client
            ProductServiceClient productServiceClient = ProductServiceClient.Create();
            // Initialize request argument(s)
            ...
            // Make the request
            Operation<ExportProductsResponse, ExportMetadata> response = productServiceClient.ExportProducts(request);

            // Poll until the returned long-running operation is complete
            Operation<ExportProductsResponse, ExportMetadata> completedResponse = response.PollUntilCompleted();
            // Retrieve the operation result
            ExportProductsResponse result = completedResponse.Result;

            // Or get the name of the operation
            string operationName = response.Name;
            // This name can be stored, then the long-running operation retrieved later by name
            Operation<ExportProductsResponse, ExportMetadata> retrievedResponse = productServiceClient.PollOnceExportProducts(operationName);
            // Check if the retrieved long-running operation has completed
            if (retrievedResponse.IsCompleted)
            {
                // If it has completed, then access the result
                ExportProductsResponse retrievedResult = retrievedResponse.Result;
            }
            // End snippet
        }

        /// <summary>Snippet for ExportProductsAsync</summary>
        public async Task ExportProductsRequestObjectAsync()
        {
            // Snippet: ExportProductsAsync(ExportProductsRequest, CallSettings)
            // Additional: ExportProductsAsync(ExportProductsRequest, CancellationToken)
            // Create client
            ProductServiceClient productServiceClient = await ProductServiceClient.CreateAsync();
            // Initialize request argument(s)
            ExportProductsRequest request = new ExportProductsRequest
            ...
            // Make the request
            Operation<ExportProductsResponse, ExportMetadata> response = await productServiceClient.ExportProductsAsync(request);

            // Poll until the returned long-running operation is complete
            Operation<ExportProductsResponse, ExportMetadata> completedResponse = await response.PollUntilCompletedAsync();
            // Retrieve the operation result
            ExportProductsResponse result = completedResponse.Result;
            ...
            string operationName = response.Name;
            // This name can be stored, then the long-running operation retrieved later by name
            Operation<ExportProductsResponse, ExportMetadata> retrievedResponse = await productServiceClient.PollOnceExportProductsAsync(operationName);
            // Check if the retrieved long-running operation has completed
            if (retrievedResponse.IsCompleted)
            {
                // If it has completed, then access the result
