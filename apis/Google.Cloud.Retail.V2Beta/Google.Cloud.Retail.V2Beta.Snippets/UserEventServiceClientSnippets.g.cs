...
namespace Google.Cloud.Retail.V2Beta.Snippets
{
    ...
    public sealed class GeneratedUserEventServiceClientSnippets
    {
        ...
        /// <summary>Snippet for ExportUserEvents</summary>
        public void ExportUserEventsRequestObject()
        {
            // Snippet: ExportUserEvents(ExportUserEventsRequest, CallSettings)
            // Create client
            UserEventServiceClient userEventServiceClient = UserEventServiceClient.Create();
            // Initialize request argument(s)
            ExportUserEventsRequest request = new ExportUserEventsRequest
            {
                Parent = "",
            ...
            // Make the request
            Operation<ExportUserEventsResponse, ExportMetadata> response = userEventServiceClient.ExportUserEvents(request);

            // Poll until the returned long-running operation is complete
            Operation<ExportUserEventsResponse, ExportMetadata> completedResponse = response.PollUntilCompleted();
            // Retrieve the operation result
            ExportUserEventsResponse result = completedResponse.Result;

            // Or get the name of the operation
            string operationName = response.Name;
            // This name can be stored, then the long-running operation retrieved later by name
            Operation<ExportUserEventsResponse, ExportMetadata> retrievedResponse = userEventServiceClient.PollOnceExportUserEvents(operationName);
            // Check if the retrieved long-running operation has completed
            if (retrievedResponse.IsCompleted)
            {
                // If it has completed, then access the result
                ExportUserEventsResponse retrievedResult = retrievedResponse.Result;
            }
            // End snippet
        }

        /// <summary>Snippet for ExportUserEventsAsync</summary>
        public async Task ExportUserEventsRequestObjectAsync()
        {
            // Snippet: ExportUserEventsAsync(ExportUserEventsRequest, CallSettings)
            // Additional: ExportUserEventsAsync(ExportUserEventsRequest, CancellationToken)
            // Create client
            UserEventServiceClient userEventServiceClient = await UserEventServiceClient.CreateAsync();
            // Initialize request argument(s)
            ExportUserEventsRequest request = new ExportUserEventsRequest
            ...
            // Make the request
            Operation<ExportUserEventsResponse, ExportMetadata> response = await userEventServiceClient.ExportUserEventsAsync(request);

            // Poll until the returned long-running operation is complete
            Operation<ExportUserEventsResponse, ExportMetadata> completedResponse = await response.PollUntilCompletedAsync();
            // Retrieve the operation result
            ExportUserEventsResponse result = completedResponse.Result;
            ...
            string operationName = response.Name;
            // This name can be stored, then the long-running operation retrieved later by name
            Operation<ExportUserEventsResponse, ExportMetadata> retrievedResponse = await userEventServiceClient.PollOnceExportUserEventsAsync(operationName);
            // Check if the retrieved long-running operation has completed
            if (retrievedResponse.IsCompleted)
            {
                // If it has completed, then access the result
