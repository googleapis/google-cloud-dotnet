...
namespace Google.Cloud.Spanner.Data.IntegrationTests
{
    ...
    public class WriteTests
    {
        ...
        private async Task<int> InsertAsync(SpannerColumnCollection values)
        {
            ...
            IDictionary<string, byte[]> record)
        {
            Assert.Equal(
                1, await InsertAsync(
                    ...
                    {
                        new SpannerColumn("BytesValue", SpannerDbType.Bytes) {Value = bytes}
                    }));
        ...
        public async Task BadTableName()
        {
            ...
            var e = await Assert.ThrowsAsync<SpannerException>(
                async () =>
                {
                    ...
                    {
                        var cmd = connection.CreateInsertCommand(
                            "badjuju", new SpannerColumnCollection
                            {
