...
namespace Google.Cloud.Spanner.V1.Tests
{
    ...
    {
        private static readonly string MaxDate = DateTime.MaxValue.Date.ToString(CultureInfo.InvariantCulture);

        private static readonly string MinDate = DateTime.MinValue.Date.ToString(CultureInfo.InvariantCulture);
        ...
        {
            var date = SpannerDate.Parse(input);
            var success = SpannerDate.TryParse(input, out SpannerDate date2);
            Assert.True(success);
        ...
        public void Inequality(string input1, string input2)
        {
            ...
            Assert.False(date1.CompareTo(date2) == 0);
            Assert.False(date2.CompareTo(date1) == 0);
        ...
        [InlineData("0001/01/01", "0001-01-01")]
        [InlineData("9999/12/31", "9999-12-31")]
        [InlineData("2022/04/18", "2022/04/18")]
        [InlineData("2022-04-18", "2022/04/18")]
        [InlineData("04-18-2022", "2022/04/18")]
        ...
        public void ToString_(string input, string expected)
        {
            ...
            Assert.Equal(expected, spannerDate.ToString());
        }
        ...
        {
            SpannerDate spannerDate = new SpannerDate(input);
        ...
        {
            Assert.Equal(SpannerDate.Parse(MaxDate), SpannerDate.MaxDate);
            Assert.Equal(SpannerDate.Parse(MinDate), SpannerDate.MinDate);
            Assert.Equal(default(SpannerDate), new SpannerDate());
        }
    }
}
