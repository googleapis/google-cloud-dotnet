...
/// </summary>
public sealed class SpannerTestDatabasePostgreSql
{
    ...
    private void CreateInstance(string projectId)
    {
        // Try to create an instance and ignore any AlreadyExists error.
        ...
        try
        {
            ...
            Instance instance = new Instance
            {
                ...
                // TODO: Check if this instance id should come from environment variable or should have any other value. 
                ConfigAsInstanceConfigName = InstanceConfigName.FromProjectInstanceConfig(projectId, "regional-us-central1"),
                DisplayName = "Test Instance.",
                NodeCount = 1
            };
            ProjectName projectName = ProjectName.FromProject(projectId);
            var completedResponse = instanceAdminClient.CreateInstance(projectName, instanceName.InstanceId, instance).PollUntilCompleted();

            if (completedResponse.IsFaulted)
            {
                Logger.DefaultLogger.Debug($"Error while creating instance: {completedResponse.Exception}");
                throw completedResponse.Exception;
            }

            Logger.DefaultLogger.Debug($"Instance {SpannerInstance} created successfully.");
        }
    ...
    private void CreatePostgreSqlDatabase()
    {
        ...
        try
        {
            ...
            if (completedResponse.IsFaulted)
            {
                Logger.DefaultLogger.Debug($"Error while creating PostgreSQL database: {completedResponse.Exception}");
                throw completedResponse.Exception;
            }

            Logger.DefaultLogger.Debug($"PostgreSql database {SpannerDatabase} created successfully.");
        }
        catch (RpcException e) when (e.StatusCode == StatusCode.AlreadyExists)
        {
            // Ignore.
