            ...
            async Task<TResult> CommitAttempt()
            {
                return await ExecuteHelper.WithErrorTranslationAndProfiling(
                    async () =>
                    {
                        ...
                        try
                        {
                            session = await (session?.WithFreshTransactionOrNewAsync(cancellationToken) ?? _connection.AcquireReadWriteSessionAsync(cancellationToken)).ConfigureAwait(false);
