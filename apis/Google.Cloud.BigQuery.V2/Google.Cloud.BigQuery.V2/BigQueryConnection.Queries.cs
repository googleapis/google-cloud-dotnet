...
namespace Google.Cloud.BigQuery.V2
{
    ...
    public sealed partial class BigQueryConnection
    {
        /// <summary>
        /// </summary>
        public BigQueryCommand CreateSqlCommand(string sql, QueryOptions queryOptions = null, GetQueryResultsOptions resultsOptions = null)
            => new BigQueryCommand
            {
    ...
    public sealed class SqlCommandOptions : BigQueryCommandOptions
    {
        ...
        internal override Task<BigQueryJob> StartJobAsync(BigQueryConnection connection, CancellationToken cancellationToken)
            => connection.GetOpenedBigQueryClient().CreateQueryJobAsync(Sql, QueryOptions, cancellationToken);

        internal override async Task<int> ExecuteNonQueryAsync(BigQueryConnection connection, CancellationToken cancellationToken)
        {
        ...
        internal override async Task<DbDataReader> ExecuteDbDataReaderAsync(BigQueryConnection connection, CommandBehavior behavior, CancellationToken cancellationToken)
        {
            var job = await StartJobAsync(connection, cancellationToken).ConfigureAwait(false);
            return new BigQueryDataReader(job, GetQueryResultsOptions, behavior == CommandBehavior.CloseConnection ? connection : null);
        }
    }
}