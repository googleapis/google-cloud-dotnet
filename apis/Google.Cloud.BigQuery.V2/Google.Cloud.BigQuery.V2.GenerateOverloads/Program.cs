...
namespace Google.Cloud.BigQuery.V2.GenerateOverloads
{
    ...
    class Program
    {
        ...
        private static IEnumerable<string> ModifyCode(ApiMethod method, IEnumerable<string> existingCode)
        {
            ...
            string regionLine = $"#region {regionLabel}";
            generatedCode.InsertRange(0, new[]
            {
                regionLine,
                "        // Code inside this region is autogenerated - do not edit manually.",
                ""
            });
            generatedCode.Add("#endregion");

            int insertionPoint = RemoveRegionAndFindInsertionPoint(regionLine, code);
        ...
        /// </summary>
        /// <param name="regionLine">Line indicating the region</param>
        /// <param name="code">Code to search in (and modify)</param>
        /// <returns>The position to insert the new region.</returns>
        private static int RemoveRegionAndFindInsertionPoint(string regionLine, List<string> code)
        {
            int regionStart = code.IndexOf(regionLine);
            if (regionStart != -1)
            {
                int regionEnd = code.IndexOf("#endregion", regionStart);
                if (regionEnd == -1)
                {
                    throw new Exception("Code contained '{regionLine}' but no #endregion");
                }
