...
namespace Google.Firestore.V1beta1
{
    ...
    /// </summary>
    public sealed partial class IndexName : IResourceName, IEquatable<IndexName>
    {
        private static readonly PathTemplate s_template = new PathTemplate("projects/{project}/databases/{database}/indexes/{index}");
        ...
        {
            GaxPreconditions.CheckNotNull(indexName, nameof(indexName));
            TemplatedResourceName resourceName = s_template.ParseName(indexName);
            return new IndexName(resourceName[0], resourceName[1], resourceName[2]);
        }
        ...
        /// </summary>
        /// <remarks>
        /// This method still throws <see cref="ArgumentNullException"/> if <paramref name="indexName"/> is null,
        /// as this would usually indicate a programming error rather than a data error.
        /// </remarks>
        /// <param name="indexName">The index resource name in string form. Must not be <c>null</c>.</param>
        ...
        public static bool TryParse(string indexName, out IndexName result)
        {
            ...
            {
                result = new IndexName(resourceName[0], resourceName[1], resourceName[2]);
                return true;
            }
            else
            {
        ...
        public IndexName(string projectId, string databaseId, string indexId)
        {
            ProjectId = GaxPreconditions.CheckNotNull(projectId, nameof(projectId));
            DatabaseId = GaxPreconditions.CheckNotNull(databaseId, nameof(databaseId));
            IndexId = GaxPreconditions.CheckNotNull(indexId, nameof(indexId));
        }
        ...
        /// <inheritdoc />
        public override string ToString() => s_template.Expand(ProjectId, DatabaseId, IndexId);
