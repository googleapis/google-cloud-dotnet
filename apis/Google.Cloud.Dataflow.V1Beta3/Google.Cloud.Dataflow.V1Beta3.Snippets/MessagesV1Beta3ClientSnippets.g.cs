...
namespace GoogleCSharpSnippets
{
    ...
    public sealed class AllGeneratedMessagesV1Beta3ClientSnippets
    {
        ...
        public async Task ListJobMessagesRequestObjectAsync()
        {
            ...
            // Iterate over all response items, lazily performing RPCs as required
            await foreach (JobMessage item in response)
            {
                // Do something with each item
                Console.WriteLine(item);
            }
            ...
            // Or iterate over pages (of server-defined size), performing one RPC per page
            await foreach (ListJobMessagesResponse page in response.AsRawResponses())
            {
                // Do something with each page of items
                Console.WriteLine("A page of results:");
                foreach (JobMessage item in page)
