...
namespace GoogleCSharpSnippets
{
    ...
    public sealed partial class GeneratedIndexServiceClientSnippets
    {
        ...
        public async Task ImportIndexRequestObjectAsync()
        {
            ...
            ImportIndexRequest request = new ImportIndexRequest
            {
                IndexName = IndexName.FromProjectLocationIndex("[PROJECT]", "[LOCATION]", "[INDEX]"),
                IsCompleteOverwrite = false,
                Config = new ImportIndexRequest.Types.ConnectorConfig(),
            };
            // Make the request
            Operation<Index, ImportIndexOperationMetadata> response = await indexServiceClient.ImportIndexAsync(request);

            // Poll until the returned long-running operation is complete
            Operation<Index, ImportIndexOperationMetadata> completedResponse = await response.PollUntilCompletedAsync();
            ...
            Operation<Index, ImportIndexOperationMetadata> retrievedResponse = await indexServiceClient.PollOnceImportIndexAsync(operationName);
