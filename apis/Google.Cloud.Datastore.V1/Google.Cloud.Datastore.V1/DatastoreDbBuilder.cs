...
namespace Google.Cloud.Datastore.V1
{
    ...
    public sealed partial class DatastoreDbBuilder : ClientBuilderBase<DatastoreDb>
    {
        ...
        private static readonly IReadOnlyList<string> RequiredEmulatorVariables = new List<string> { EmulatorHostVariable }.AsReadOnly();
        private static readonly IReadOnlyList<string> AllEmulatorVariables = new List<string> { EmulatorHostVariable, EmulatorProjectVariable }.AsReadOnly();
        ...
        {
            var emulatorBuilder = (DatastoreDbBuilder) MaybeUseEmulator();
            if (emulatorBuilder is object)
            {
                return emulatorBuilder.PrepareBuilder();
            }
        ...
        /// <inheritdoc />
        protected override IReadOnlyList<string> RequiredEmulatorEnvironmentVariables => RequiredEmulatorVariables;

        /// <inheritdoc />
        protected override IReadOnlyList<string> AllEmulatorEnvironmentVariables => AllEmulatorVariables;

        /// <inheritdoc />
        protected override ClientBuilderBase<DatastoreDb> BuildEmulatorClientBuilder(Dictionary<string, string> environment) =>
            new DatastoreDbBuilder
            {
                Endpoint = environment[EmulatorHostVariable],
                Settings = Settings,
                ProjectId = ProjectId ?? environment[EmulatorProjectVariable],
