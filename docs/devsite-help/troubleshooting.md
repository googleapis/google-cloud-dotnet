...
- `Reason` - It is a constant value and indicates the proximate cause of 
the error. Error reasons are unique within a particular domain of errors.

- `Domain` - It is the logical grouping to which the error belongs. It is 
...
- `Metadata`- It is the additional structured details about the error. 
It can help in identifying the exact cause of the error.
...
### How can I get RpcException from SpannerException?
...
value may be null if the `SpannerException` was not caused by `RpcException`.

`RpcException` property of `SpannerException` can be used like:
...
{
   var rpcException = ex.RpcException;   
   if(rpcException != null)
   {
     // Use rpcException to fetch domain, reason and metadata from its ErrorInfo for further use.
   }
}
```
