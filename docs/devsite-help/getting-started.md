...
- If you have a Google Cloud support contract, please create an issue in the [support console](https://cloud.google.com/support/).
- If you have a question which is specific to a Google Cloud Library for .NET,
  for example how to specify credentials in a particular way, please file an issue in the
  [google-cloud-dotnet GitHub repository](https://github.com/googleapis/google-cloud-dotnet).
- If you have an API-specific question, please consult the support pages for that particular API.
  The maintainers of the Google Cloud Libraries for .NET don't have experience in all APIs; they
  are able to help with aspects that are specific to the .NET libraries, but can't provide guidance
  on individual API methods. (If you're not sure whether a problem is due to the library or the underlying
  API, it's fine to file an issue in the repository linked above. If it turns out to be an API issue, the
  library maintainers will guide you to the API-specific support page.)
